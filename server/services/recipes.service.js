...
class RecipesService {
  ...
  async getRecipeById(id) {
    return this.collection.findOne({ id: parseInt(id, 10) });
  }
    ...
    return this.collection
      .find({
        ingredients: {
          $elemMatch: {
            nom: {
              $regex: matchExact ? `^${ingredient}$` : `^${ingredient}.*`,
            },
          },
        },
      })
  ...
  async deleteRecipeById(id) {
    return this.collection.findOneAndDelete({ id: parseInt(id, 10) });
  }
  ...
  async addNewRecipe(recipe) {
    ...
    return this.collection.insertOne(newRecipe);
  }
