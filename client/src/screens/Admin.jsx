...
export default function Admin() {
  ...
  const renderContent = () => {
    switch (render) {
      case 'recipes':
        return recipes.map((recette) => <AdminRecipe key={recette.id} recipe={recette} handleDelete={deleteRecipe} />);
      case 'contacts':
        return contacts.map((contact) => (
          <AdminContact key={contact.id} contact={contact} handleDelete={deleteContact} />
        ));
      default:
